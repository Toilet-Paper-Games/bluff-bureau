...
export function mapRuntimeRole(role: RuntimeParticipant["role"] | SurfaceContext["surfaceKind"]): DomainParticipantRole {
  ...
  throw new Error(`Unsupported Bluff Bureau participant role: ${role}`);
}
...
export class TpgRuntimeAdapter implements RuntimePort {
  constructor(private readonly api: IframeGameRuntimeApi<BluffBureauState, PlayerIntentState>) {}

  context() { return mapContext(this.api.context()); }
  participants() { return this.api.participants().map(mapParticipant); }
  sharedState() {
    const snapshot = this.api.getSharedStateSnapshot();
    return { value: decodeBluffBureauState(snapshot.value), revision: snapshot.revision };
  }
  ownPlayerState() {
    const snapshot = this.api.getPlayerStateSnapshot();
    return { value: decodePlayerIntentState(snapshot?.value), revision: snapshot?.revision ?? 0 };
  }
  async writeSharedState(value: BluffBureauState, expectedRevision: number) {
  ...
  async writeOwnPlayerState(value: PlayerIntentState, expectedRevision: number) {
    return mapMutation(await this.api.setPlayerState(value, undefined, { expectedRevision }));
  }
  ...
  subscribePlayerState(listener: Parameters<RuntimePort["subscribePlayerState"]>[0]) {
    for (const participant of this.api.participants()) {
      ...
      if (snapshot?.value !== undefined) {
        listener(participant.id, { value: decodePlayerIntentState(snapshot.value), revision: snapshot.revision });
      }
    }
  ...
  subscribeContext(listener: Parameters<RuntimePort["subscribeContext"]>[0]) {
    return this.api.subscribeShellEvent<SurfaceContext>("runtime:context", (context) => listener(mapContext(context)));
  }
  subscribeLifecycle(listener: Parameters<RuntimePort["subscribeLifecycle"]>[0]) {
    return this.api.subscribeLifecycle(listener);
  }
  reportReady() { return this.api.reportLoading(true); }
  openSettings() { return this.api.openSettings(); }
  returnToLobby() { return this.api.returnToLobby(); }
  reportAnalytics(event: Parameters<RuntimePort["reportAnalytics"]>[0]) { return this.api.reportAnalytics(event); }
}
